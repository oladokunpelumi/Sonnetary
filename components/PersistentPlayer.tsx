...
const PersistentPlayer: React.FC = () => {
  ...
  // Generate consistent waveform bars (not random each render)
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[100] bg-[#120809] border-t border-primary/20 h-20 px-4 md:px-8 flex items-center justify-between shadow-[0_-5px_20px_rgba(0,0,0,0.5)] transition-all">
      {/* Track Info */}
      <div className="flex items-center gap-4 w-1/4 min-w-[200px]">
        <div className="h-12 w-12 rounded bg-slate-800 relative overflow-hidden group shrink-0">
          <div
        ...
        <div className="flex flex-col overflow-hidden">
          <span className="text-white font-bold text-sm truncate">{activeSong.title}</span>
      ...
      <div className="flex flex-col items-center flex-1 max-w-2xl px-4">
        <div className="flex items-center gap-6 mb-1">
          <button onClick={skipPrev} className="text-slate-400 hover:text-white transition-colors">
            <span className="material-symbols-outlined text-xl">skip_previous</span>
          </button>
          <button
            onClick={togglePlay}
            disabled={!hasAudio || isPreviewLocked}
            className={`h-10 w-10 rounded-full flex items-center justify-center transition-transform ${hasAudio && !isPreviewLocked ? 'bg-white text-primary hover:scale-105' : 'bg-slate-700 text-slate-500 cursor-not-allowed'}`}
          >
            <span className="material-symbols-outlined fill">{isPlaying ? 'pause' : 'play_arrow'}</span>
          </button>
          <button onClick={skipNext} className="text-slate-400 hover:text-white transition-colors">
            <span className="material-symbols-outlined text-xl">skip_next</span>
          </button>
        </div>

        {/* Waveform / Preview lock */}
        <div className="w-full flex items-center gap-3 relative">
          ...
          {isPreviewLocked ? (
            ...
            <div className="flex-1 h-8 flex items-center justify-center gap-2 bg-primary/10 border border-primary/20 rounded-lg px-3">
              ...
              <Link
                to="/create"
                className="ml-auto text-[10px] font-bold bg-primary text-white px-2 py-0.5 rounded hover:bg-red-600 transition-colors uppercase tracking-wider"
              >
                Get Yours
              </Link>
            </div>
            ...
            <div
              className="h-8 flex-1 flex items-center gap-[2px] opacity-60 cursor-pointer"
              onClick={(e) => {
            ...
            >
              {waveformBars.map((height, i) => (
                <div
                  key={i}
                  className={`w-1 rounded-full transition-colors ${i < activeBarIndex ? 'bg-primary' : 'bg-slate-700'}`}
                  style={{ height: `${height}%` }}
                />
              ))}
            </div>
